...
select CONCAT(first_name,trim(last_name)) as full_name,title,levels from employee order by levels DESC 

-- Q2: Which countries have the most Invoices?
select count(*) as C ,billing_country from invoice group by billing_country order by c desc

-- Q3: What are top 3 values of total invoices?
select trunc(total) from invoice order by total desc limit 3
...
-- 	   highest sum of invoices totals. Return both the city name & sum of all invoices total. 
select billing_city,Round(sum(total)) as invoice_total from invoice group by billing_city order by invoice_total desc 
...
--     listeners. Return your lsit ordered alphabetically by email starting with A.
